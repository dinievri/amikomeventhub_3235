...
<div class="max-w-2xl mx-auto">
    <div class="mb-6">
        ...
        <p class="text-sm text-slate-500">Perbarui nama kategori event yang sudah ada.</p>
    </div>
    ...
    <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
        <form action="{{ route('admin.categories.update', $category->id) }}" method="POST">
            ...
            <div class="mb-5">
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $category->name) }}" required placeholder="Contoh: Webinar IT" class="w-full rounded-xl border border-slate-200 p-3 text-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 outline-none transition bg-slate-50 @error('name') border-red-500 @enderror">
                @error('name')
